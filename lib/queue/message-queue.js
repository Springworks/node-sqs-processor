...
exports.create = function(sqs_handler, logger) {
  ...
  function receiveQueueMessages(queue_name, callback) {
    var queue_url_params = sqs_handler.getQueueUrlParams(queue_name);

    logger.debug('receiveQueueMessages, %s:', queue_name);

    // Get queue URL first and then load messages
    sqs_handler.getQueueUrl(queue_url_params, function(err, queue_url) {
      if (err) {
        logger.warn(err, 'Error loading url for queue %s:', queue_url_params.QueueName);
        callback(err, null);
        return;
      }
      ...
      sqs_handler.receiveMessageBatch(queue_url, function(err, data) {
        if (err) {
          ...
          // No messages returned; not an error, but just an empty batch
          logger.debug(data, 'No messages in SQS response');
          callback(null, []);
        }
      });
    });
